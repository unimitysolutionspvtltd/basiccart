...
class CartForm extends FormBase {
  ...
  foreach ($cart['cart_quantity'] as $nid => $quantity) {
    ...
    //print_r($cart['cart'][$nid]->getTranslation('en')->get('title')->getValue()); die;
  }
...
//print_r($form_state->getValue('cartcontents')); die;
  if($config->get('quantity_status')) {
    ...
    drupal_set_message(t('No change to update'),'warning');
  }
...
public function get_quantity_prefix_suffix($nid,$langcode) {
  ...
  //$prefix .= '    <span class="basiccart-cart-node-summary">' . $title . '</span>';
  $prefix .= '  </div>';
  $prefix .= '  <div class="cell basiccart-cart-unit-price"><strong>' . $unit_price . '</strong></div>';
  //$prefix .= '  <div class="cell basiccart-cart-x">x</div>';
